#this code is AI made.  Human only handled some of the difficult parts and cleaned up the code to make it readable. :))))
...
# --- SICAKLIK / KÜRESEL ISINMA AYARLARI ---
# normalize (0-1) sıcaklık değerlerinin ekranda gösterileceği gerçek derece aralığı
TEMP_DISPLAY_MIN_C = -15.0
TEMP_DISPLAY_MAX_C = 48.0
GLOBAL_WARMING_RATE = 0.010   # ortalama kirlilik arttıkça sıcaklığın ne kadar yükseleceği (offset/tick katsayısı)
GLOBAL_WARMING_MAX = 0.35     # sıcaklığa eklenebilecek maksimum ısınma offseti (normalize birim)
CLIMATE_UPDATE_INTERVAL = 240  # kaç tick'te bir iklim (küresel ısınma) yeniden hesaplansın
...
# --- TANRI'NIN YARATICILIĞI (SAMPLING TEMPERATURE) ---
# Model çıktısı (13 boyutlu y_vector'ün logit'leri) sigmoid'e girmeden önce bu
# sıcaklığa göre rastgele gürültüyle bozuluyor. 0'a yakın -> Tanrı modelin "en olası"
# tasarımını neredeyse hiç sapmadan üretir (kararlı/tutucu). Yüksek değerler -> aynı
# bölge koşullarında bile her seferinde çok farklı, "yaratıcı"/kaotik tasarımlar çıkar.
GOD_TEMPERATURE_DEFAULT = 0.6
GOD_TEMPERATURE_MIN, GOD_TEMPERATURE_MAX = 0.0, 2.5
GOD_TEMPERATURE_STEP = 0.1
GOD_TEMPERATURE_NOISE_SCALE = 2.2  # sıcaklık birimini logit-gürültü std'sine çeviren katsayı
...
def temp_norm_to_celsius(t):
    """0-1 aralığındaki normalize sıcaklığı okunabilir bir °C değerine çevirir."""
    return TEMP_DISPLAY_MIN_C + clamp01(t) * (TEMP_DISPLAY_MAX_C - TEMP_DISPLAY_MIN_C)


def temp_to_heat_color(t):
    """Isı haritası görünümü için 0-1 sıcaklığı mavi(soğuk) -> kırmızı(sıcak) renk gradyanına çevirir."""
    t = clamp01(t)
    if t < 0.5:
        k = t / 0.5
        r = int(30 + k * 60)
...
class World:
    def __init__(self):
        ...
        self.global_warming = 0.0  # kirlilik biriktikçe yükselen, tüm haritaya eklenen sıcaklık offseti
    ...
    def temperature_at(self, c, r):
        return clamp01(self.grid[r][c].base_temp + SEASON_TEMP_MOD[self.season_index()] + self.global_warming)

    def humidity_at(self, c, r):
        # küresel ısınma nemi de kurutur (buharlaşma artar, ortalama nem düşer)
        return clamp01(self.grid[r][c].base_humid + SEASON_HUMID_MOD[self.season_index()] - self.global_warming * 0.4)

    def water_temperature_at(self, c, r):
        cell = self.grid[r][c]
        base = clamp01(cell.base_temp + SEASON_TEMP_MOD[self.season_index()] * 0.4 + self.global_warming * 0.5)
        return clamp01(base - cell.depth * 0.2)

    def temperature_celsius_at(self, c, r):
        return temp_norm_to_celsius(self.temperature_at(c, r))

    def update_climate(self):
        """Ortalama kirlilik yüksekse haritayı yavaşça ısıtır (küresel ısınma), kirlilik düşükse
        çok yavaş soğutup dengeye döner."""
        if self.tick % CLIMATE_UPDATE_INTERVAL != 0:
            return
        pressure = (self.average_pollution - 0.15) * GLOBAL_WARMING_RATE
        self.global_warming = clamp(self.global_warming + pressure, 0.0, GLOBAL_WARMING_MAX)
...
class AIGod:
    ...
    def __init__(self, sim):
        ...
        self.temperature = GOD_TEMPERATURE_DEFAULT

    def adjust_temperature(self, delta):
        self.temperature = clamp(self.temperature + delta, GOD_TEMPERATURE_MIN, GOD_TEMPERATURE_MAX)
    ...
    def maybe_decide(self):
        ...
        with torch.no_grad():
            logits = self.model(torch.tensor([x_vec], dtype=torch.float32))[0]
            if self.temperature > 0.0:
                noise = torch.randn_like(logits) * (self.temperature * GOD_TEMPERATURE_NOISE_SCALE)
                logits = logits + noise
            y_vec = torch.sigmoid(logits).tolist()
        ...
        self.last_decision_text = (
            ...
            f"{genome.habitat.name}/{genome.diet.name}, {spawned} canlı hediye edildi (sıcaklık {self.temperature:.1f})"
        )
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    show_temp_view = not show_temp_view
                elif event.key in (pygame.K_MINUS, pygame.K_KP_MINUS, pygame.K_COMMA):
                    sim.god.adjust_temperature(-GOD_TEMPERATURE_STEP)
                elif event.key in (pygame.K_EQUALS, pygame.K_KP_PLUS, pygame.K_PERIOD):
                    sim.god.adjust_temperature(GOD_TEMPERATURE_STEP)
        ...
        for r in range(r0, r1):
            ...
            for c in range(c0, c1):
                cell = row[c]
                if show_temp_view:
                    color = temp_to_heat_color(sim.world.temperature_at(c, r))
                else:
                    color = TERRAIN_BASE_COLOR.get(cell.terrain, (90, 90, 90))
        ...
        mouse_x, mouse_y = pygame.mouse.get_pos()
        hover_wx = cam_x + mouse_x / zoom
        hover_wy = cam_y + mouse_y / zoom
        hover_cell, hover_c, hover_r = sim.world.cell_at(hover_wx, hover_wy)
        hover_temp_c = sim.world.temperature_celsius_at(hover_c, hover_r)

        avg_temp_c = temp_norm_to_celsius(sim.world.average_temperature + sim.world.global_warming)
        warming_pct = (sim.world.global_warming / GLOBAL_WARMING_MAX) * 100 if GLOBAL_WARMING_MAX else 0
        ...
        hud_lines = [
            ...
            f"Tanrı Yaratıcılığı (Sıcaklık): {sim.god.temperature:.1f}  ( , / . ile ayarla — 0=tutucu, {GOD_TEMPERATURE_MAX:.1f}=kaotik)",
            f"Tanrı: {sim.god.last_decision_text}",
            f"Ort. Sıcaklık: {avg_temp_c:.1f}°C   Küresel Isınma: %{warming_pct:.0f}   İmleç altı: {hover_temp_c:.1f}°C ({hover_cell.terrain.name})",
            "Yön tuşları/WASD: kaydır   Fare tekerleği: zoom   SPACE: turbo   T: ısı haritası   , / . : Tanrı sıcaklığı   R: sıfırla   ESC: çıkış",
        ]
        ...
        if show_temp_view:
            badge = big_font.render("ISI HARİTASI GÖRÜNÜMÜ (T)", True, (255, 160, 90))
            screen.blit(badge, (14, screen_h - 40))
